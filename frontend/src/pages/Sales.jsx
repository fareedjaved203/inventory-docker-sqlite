...
function Sales() {
  ...
  const handleDateChange = (e) => {
    ...
    // Convert YYYY-MM-DD to DD/MM/YYYY format for the backend
    if (date) {
      const [year, month, day] = date.split('-');
      const formattedDate = `${day}/${month}/${year}`;
      // Fetch sales with the formatted date
      queryClient.invalidateQueries(['sales', formattedDate]);
    } else {
      queryClient.invalidateQueries(['sales', '']);
    }
  };
  ...
  const deleteSale = useMutation(
    ...
    },
    {
      ...
      onError: (error) => {
        setDeleteError(error.response?.data?.error || 'An error occurred while deleting the sale');
      }
    }
  );
    ...
    <div className="p-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div className="flex items-center gap-2 flex-wrap">
          <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-primary-800">Sales</h1>
          {showPendingPayments && (
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {(selectedDate) && (
              <button
                ...
                className="px-3 py-2 text-sm border border-primary-200 rounded-lg hover:bg-primary-50 text-primary-600"
              >
                Clear
              </button>
            )}
      ...
      <div className="mt-4 flex justify-center">
        <div className="flex space-x-2">
          ...
          <span className="px-4 py-2 bg-primary-50 border border-primary-200 rounded-lg text-primary-800">
            Page {currentPage} of {Math.ceil((sales?.total || 0) / itemsPerPage)}
          </span>
          <button
            onClick={() => setCurrentPage((prev) => prev + 1)}
            disabled={currentPage >= Math.ceil((sales?.total || 0) / itemsPerPage)}
            className="px-4 py-2 border border-primary-200 rounded-lg disabled:opacity-50 text-primary-700 hover:bg-primary-50"
          >
            Next
          </button>
          ...
          <div className="bg-white p-6 rounded-lg w-full max-w-lg h-[90vh] shadow-xl border border-gray-200 flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
                          ...
                          {filteredProducts?.map((product) => (
                            ...
                            >
                              <div>
                                ...
                                <div className="text-sm text-gray-600">{product.quantity} in stock</div>
                              </div>
                              <div className="text-primary-600 font-medium">Rs.{product.price}</div>
                            </div>
                          ))}
                        </div>
                      )}
              ...
              <div className="border border-primary-100 rounded-lg p-4 bg-primary-50">
                <h3 className="font-medium mb-2 text-primary-800">Sale Items</h3>
                {saleItems.length === 0 ? (
                ...
                <div className="mt-2 pt-2 border-t border-primary-200">
                  <div className="font-medium text-primary-800">Total: {formatPakistaniCurrency(calculateTotal())}</div>
                </div>
              </div>
      ...
      <DeleteModal
        ...
        itemName={saleToDelete ? `sale from ${new Date(saleToDelete.saleDate).toLocaleDateString()}` : ''}
        error={deleteError}
      />
