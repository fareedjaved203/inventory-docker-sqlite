...
CMD ["sh", "-c", "node ensure-data-dir.js && echo 'Initializing database...' && node init-db.js && echo 'Adding returns tables...' && node add-returns-migration.js && echo 'Adding original amount field...' && node add-original-amount.js && echo 'Adding refund tracking...' && node add-refund-fields.js && echo 'Adding discount field...' && node add-discount-migration.js && echo 'Adding damaged quantity field...' && node add-damaged-quantity.js && echo 'Adding low stock threshold field...' && node add-low-stock-threshold.js && echo 'Adding purchase price field...' && node add-purchase-price.js && echo 'Handling migrations...' && (npx prisma migrate resolve --applied 20241201000000_add_discount_to_sales || echo 'Migration already handled') && (npx prisma migrate resolve --applied 20241201000001_add_damaged_quantity || echo 'Migration already handled') && echo 'Starting application...' && node src/index.js"]